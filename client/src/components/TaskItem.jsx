  ...
  return (
    <div className='bg-white rounded-xl p-5 flex flex-col gap-4'>
      {loading && <Spinner />}
        ...
        {/* edit Btn */}
        <Link className="text-white p-1 bg-orangeColor text-sm" to="/dashboard/editTask" state={task}>edit</Link>
        {/* delete Btn */}
        <Link className="bg-deleteColor text-white text-sm p-1" onClick={()=>deleteT(task._id)}>delete</Link>
      </div>
      </div>
        {/* task name */}
        <h3>{task.task}</h3>
        ...
        {/* range input */}
        <input type='range' min={"0"} max={"10"} value={pv}  className='rangeInput' onChange={changeProgress} />
        {/* created at */}
