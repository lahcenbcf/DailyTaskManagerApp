...
function Register() {
  ...
  return (
    <div className="px-4 flex items-center justify-center bg-bg-color h-screen">
      {loading && <Spinner />}
      {success && (
        <div className="absolute top-4 right-[50%] translate-x-[50%] w-fit text-white font-bold alert alert-success">
      ...
      <form
        onSubmit={submitForm}
        className="max-w-md w-full rounded-md p-4 shadow-lg flex flex-col items-center relative bg-white"
      >
        {/* close Button */}
        <IoMdClose className="absolute top-3 right-3" />
        <h2 className="text-black font-bold my-4">
          Sign Up To{" "}
          <span className="font-extrabold text-primary">Simplr.</span> !
        </h2>
        ...
        <div className="relative w-full mx-auto flex justify-center">
          ...
          {!visible ? (
            <IoEyeOffOutline
              className="absolute right-[15%] top-[50%] -translate-y-[50%]"
              onClick={handleVisible}
            />
          ) : (
            <IoEyeOutline
        ...
        {/* forgot pass */}
        <p className="text-primary font-bold self-start md:ml-8">
          have an account ?{" "}
        ...
        <button
          type="submit"
          className="w-full lg:w-11/12 py-2 bg-primary text-white font-bold my-4"
        >
          create account
        </button>
      </form>
